...
export function useRTDB<T = any>(path: string, debounceOptions?: WatchDebouncedOptions<boolean>) {
  ...
  function push<TItem = T>(item: TItem, isEasykey: boolean = false) {
 ...
 * @returns vue ref 的變數 | null 找不到資料庫 | undfined loading 中
 */
export function usePromiseRTDB<T = any>(path: string, debounceOptions?: WatchDebouncedOptions<boolean>) {
  return new Promise<{
    data: Ref<T>,
    push: Function
  }>((resolve, reject) => {
